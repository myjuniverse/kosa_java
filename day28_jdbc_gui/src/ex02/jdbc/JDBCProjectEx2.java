...
public class JDBCProjectEx2 extends JFrame implements ActionListener{
	...
	public JDBCProjectEx2(){ // 생성자함수 초기화 담당
		//component 등록
	...
	public void dbconnect() {
		try {
			...
			psInsert = conn.prepareStatement(sqlDelete);
			psDelete = conn.prepareStatement(sqlInsert);
			psTotal = conn.prepareStatement(sqlSelect);
			psSearch = conn.prepareStatement(sqlSearch);
			
			psTotalScroll = conn.prepareStatement(sqlSelect, ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE);					
			psSearchScroll = conn.prepareStatement(sqlSearch, ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE);
